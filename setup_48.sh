...
# g_audio config: 48kHz 16bit stereo
# c_ssize=2 = 16bit (2 bytes), c_srate=48000
cat > /etc/modprobe.d/g_audio.conf << 'EOF'
options g_audio c_chmask=3 p_chmask=0 c_srate=48000 p_srate=48000 c_ssize=2 p_ssize=2
EOF

# ── 4. Configure PipeWire for 48kHz ──
echo ""
...
cat > "$PW_CONF_DIR/48khz.conf" << 'EOF'
context.properties = {
...
echo 'net.ipv4.tcp_retries2 = 5' | tee /etc/sysctl.d/99-tcp-retries.conf
...
echo ""
echo " Audio: 48000:16:2 throughout"
echo ""
