...
def load_or_init_password() -> str:
    """Load hash from file. If missing or invalid, seed default and return it."""
...
class ClientBridge:
    ...
    def _on_pw_broadcast(self, h: str):
        """Called from PasswordListener thread when server Pi sends new hash."""
        save_password_hash(h)
        self._pw_hash = h
        log.info("Password updated from server broadcast")

    def _handle_pw_check(self, payload: bytes):
        """ESP sent a raw password — hash and compare, reply with result."""
    ...
    def _handle_rename(self, payload: bytes):
        """ESP sent a new hostname — sanitize, apply, restart snapclient."""
        ...
        try:
            ...
            if self.mode == MODE_SYNC and snapclient_is_running():
                ...
                snapclient_start()
                time.sleep(2)
                # Re-resolve client ID under new name
                self.client_id = None
                self._last_rpc_attempt = 0.0
            # Push updated state so zone label refreshes on ESP
            self.send_state(force=True)
            log.info("Hostname changed to '%s'", new_name)
